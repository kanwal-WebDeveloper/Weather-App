...
function showError(message = "Invalid city name or network issue") {
    ...
    weatherContainer.innerHTML = `<div class="loading-placeholder"><i class="fas fa-cloud-rain" style="font-size: 2rem;"></i><p>⚠️ ${message}</p><p style="font-size:12px;">دوبارہ کوشش کریں</p></div>`;
}
...
async function updateWeatherForCity(cityName) {
    if (!cityName || cityName.trim() === "") {
        showError("براہ کرم کوئی شہر لکھیں");
        return;
    }
    ...
    try {
        ...
        if (!currResp.ok) {
            if (currResp.status === 404) showError("City not found! صحیح شہر لکھیں");
...
cityInput.addEventListener("keypress", (e) => {
    if (e.key === "Enter") {
        ...
        else showError("City name likhen");
    }
});
